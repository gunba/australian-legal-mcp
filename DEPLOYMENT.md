...
Software 0.19.5 implements the next hard-cut host-tools transaction, but this
...
```bash
gh release download v0.19.5 --repo gunba/australian-legal-mcp \
  --pattern 'legal-mcp-*' --pattern SHA256SUMS
sha256sum --check SHA256SUMS
```

This v0.19.5 command is valid only after that immutable patch release exists.
...
not relabel those historical evidence sets as v0.19.5.
...
the immutable v0.19.5 release exists, independently verify its checksums,
...
```bash
sudo infra/linode/install-host.sh --upgrade-host-tools --version 0.19.5
```

If interrupted, recover from the same exact bundle before continuing:

```bash
sudo infra/linode/install-host.sh --recover-host-tools --version 0.19.5
```
