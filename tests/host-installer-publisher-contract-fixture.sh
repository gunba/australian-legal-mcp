...
grep -Fq 'configured_dark_recovery_ready' "$installer"
...
grep -Fq 'CUTOVER_PERMIT=/run/legal-mcp/flat-int8-cutover-starting' "$installer"
grep -Fq 'CUTOVER_ARM=/run/legal-mcp/flat-int8-cutover-start-armed' "$installer"
grep -Fq 'cutover_release_binding_recoverable' "$installer"
grep -Fq 'write_cutover_permit "$$" "$process_start"' "$installer"
grep -Fq 'strict_dispatch_permit "$CUTOVER_PERMIT" update-image --flat-int8-cutover' "$installer"
grep -Fq 'cmp --silent "$CUTOVER_PERMIT" "$CUTOVER_ARM"' "$installer"
...
grep -Fq 'drain_saved_host_tool_processes "$HOST_TOOLS_TRANSACTION"' "$installer"
grep -Fq 'host-tool-launcher-device-inode' "$installer"
...
grep -Fq 'LEGAL_MCP_FLAT_INT8_CUTOVER_TRANSACTION_V1' "$image_updater"
grep -Fq 'cutover_call_host_deploy cutover-activate' "$image_updater"
grep -Fq 'cutover_call_host_deploy cutover-rollback' "$image_updater"
grep -Fq 'cutover_call_host_deploy cutover-commit' "$image_updater"
