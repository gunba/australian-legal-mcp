...
old_image="ghcr.io/gunba/australian-legal-mcp@sha256:$(printf 'c%.0s' {1..64})"
target_image="ghcr.io/gunba/australian-legal-mcp@sha256:$(printf 'd%.0s' {1..64})"
old_image_id="sha256:$(printf 'e%.0s' {1..64})"
target_image_id="sha256:$(printf 'f%.0s' {1..64})"
...
install -d -o root -g root -m 0755 "$bundle/infra/hosting" "$bundle/scripts"
...
run_cutover() {
  ...
  if "$update_entry" --flat-int8-cutover \
    ...
    --template "$bundle/infra/hosting/legal-mcp.container.template"; then
    status=0
  else
    status=$?
  fi
...
run_recovery() {
  local status
  restore_public_launchers
  if "$update_entry" --recover --flat-int8-cutover; then status=0; else status=$?; fi
  restore_public_launchers
  return "$status"
}
