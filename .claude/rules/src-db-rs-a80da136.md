...
- [OF-01 L223] canonical_url is synthesised from doc_id by direct substitution into the ATO URL pattern; href is not stored separately so the link always reflects the current doc_id.
...
- [SL-12 L137] document_assets stores image asset bytes inline: keyed by asset_ref (ato-image://<doc_id>/<ordinal>), referencing doc_id, with media_type, alt, title, sha256, and the raw bytes in a 'data' BLOB column. The release artifact ships the BLOBs in ato.db.zst; there is no on-disk live/assets/ tree.
- [SL-10 L152] doc_anchors stores in-document anchors, sister-document links, and historical-version pointers extracted at build time; get_doc_anchors also includes reverse citations from the citations table.
- [SL-11 L166] citations is a derived reverse-citation index populated from inline [doc:X] markers. It is keyed by source_chunk_id plus target_doc_id, indexed by target_doc_id, collapses qualifiers to the base doc_id, and skips self-citations.
- [SL-04 L181] Both title_fts and chunks_fts are FTS5 virtual tables using tokenize='porter unicode61 remove_diacritics 2' for stemmed, diacritic-insensitive English legal text search.
