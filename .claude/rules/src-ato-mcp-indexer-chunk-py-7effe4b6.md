...
- [IB-22 L572] _pack_chunks tracks raw word count, not sum-of-int-truncated approx_tokens(block.text), so the projected token total matches approx_tokens of the joined chunk exactly. Per-block int(words * 1.3) truncation otherwise accumulates ~1 token of drift per block, letting chunks land 5–30 tokens over max_tokens when packed from many small blocks.
