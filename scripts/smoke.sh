...
if [[ -z "$source_data_dir" ]]; then
	source_data_dir="${ATO_MCP_DATA_DIR:-$HOME/.local/share/ato-mcp}"
fi
...
if [[ -d "$live_src" ]]; then
	...
	bad "live corpus not found at $live_src — install one with the same ATO_MCP_DATA_DIR using: ato-mcp update"
	rm -rf "$workdir"
	echo ""
