...
#   3. `ato-mcp publish-release` verifies and uploads ato.db.zst, ato.ann,
#      optional signature, then manifest.json strictly last
#
# Prereqs:
...
"$ATO_MCP_BIN" package-corpus \
	...
	>/dev/null
if [ ! -f "$RELEASE_DIR/ato.ann" ]; then
	echo "missing ANN sidecar: $RELEASE_DIR/ato.ann" >&2
	exit 1
fi
...
echo "=> publishing database, ANN sidecar, signature, then manifest"
